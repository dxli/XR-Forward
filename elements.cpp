...
//get real part of an element (ele0) at a given photon energy en0
{
    ...
    fn(string(nffdir)+"mass_abs_coeffs.dat"),
    ...
    while (i<ele.size() && isalpha(ele.at(i))){
        if (i) ele.at(i)=tolower(ele.at(i));
        else ele.at(i)=toupper(ele.at(i));
        if (i++>=1) break;
    }
...
compound::compound(string c0,double l0,double rho0)
//compound, get the real part of form factor and absorption, compound chemical formula in c0, l0 in angstrom, bulk density in rho0 (g/cm^3)
// modified to accept wavelength instead of energy, this is different from element constructor, where energy is supplied
{
    density=rho0;
    double en0=E_to_l(l0); // from lambda to energy, using the same E_to_l function
    rho_el=0.;
    beta=0.;
    vector<string> el0;
    string c1=c0;
    ...
    while (j<c1.size()) { //parse the formula
        ...
        if (num.size()) n0.push_back(atoi(num.c_str()));
        else n0.push_back(1);
    }
...
// Element symbol, use density from data_base
    ...
    nk=complex<double>(2.*re_classical*lambda*lambda/(2.*M_PI)*rho_el,-2.*beta);
}

void compound::rho_scale(double rho0)
        //scale parameters to rho0 from 1, supposed all in g/cm^3
{
        double rho00(1.0e-24); //compounds initial density fixed to 1.0 g/cm^3
        if(els.size()==1) rho00 = els.at(0).density; //elements initial from tabled value
        rho0 /=rho00;
        rho *= rho0;
        rho_el *= rho0;
        nk *= rho0;
